...
class Joint(object):
    ...
    def __init__(self, p1, p2, k, length=None):
        ...
        if math.isnan(self._dm1):
            self._dm1 = k
        if math.isnan(self._dm2):
            self._dm2 = k
...
class World(object):
    ...
    def __init__(self, n_pos, ground=restrictions.ground(), gravity=Vector2D(0, -9.8), debug=False):
        self.n_pos = n_pos
        self._ground = ground
        self.G = gravity
        self.debug = debug
    ...
    def step(self, dt):
        ...
        for i in range(0, self.n_pos):
            for j in self._joints:
                ...
                if dl != 0 and self.debug:
                    print(f'{i} l: {l} dl: {dl}, r {r}')
                j.p1.pos += r * j._dm2
                j.p2.pos -= r * j._dm1
        ...
        if self.debug:
            ...
            for i,p in enumerate(self._points):
                print(f'{i} {p.pos.x}:{p.pos.y}, {p.p_pos.x}:{p.p_pos.y}')
            print('-----------')
